...
K = gen_key(symboles)
custom_dict = gen_key(symboles)
M = text[12000:12100]
C = chiffrer(M, K,custom_dict)

print("M ="+"\""+M+"\"")
print("\nLongeur du message M =",len(M))
print("\nDivision en symboles =", M_vers_symboles(M, K,custom_dict))
print("\nNombre de symboles =", len(M_vers_symboles(M, K,custom_dict)))
print("\nC = "+"\""+C+"\"")
print("\nLongeur du cryprogramme C en bits =",len(C))
print("\nLongeur du cryprogramme C en octets =",len(C)//8)

""" L'idée est de créer un immense cryptogramme à partir d'un gros échantillon de texte français et d'analyser les fréquences 
d'apparition des blocs de bytes et les comparer avec la fréquence d'apparition des symboles dans la liste de symboles fixée 
et trouvée dans un grand échantillon de texte français pour créer une clé espérée qui devrait correspondre à la clé secrète originale qu'on ne connait pas"""
...
def create_guess_key(ciphertext):
    ...
    reference_freq_dist = Counter(M_vers_symboles(text, symboles, symboles))
    ...
    # On met les symboles les plus fréquent avec les bytes les plus fréquents
    reversed_guess_key = {}
...
def decrypt(C):
    ...
    big_ciphertext = chiffrer(text,K,dictionaire)
    guess_key = create_guess_key(big_ciphertext)

    # Remplacer chaque bytes avec son symbole correspondant selon la clé espérée
    decrypted_text = []
    for chunk in chunks:
...
M = text[12000:12100] #On peut tester n'importe quel extrait
ciphertext = chiffrer(M, K, dictionaire)
